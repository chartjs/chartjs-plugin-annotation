describe('Polygon annotation', function() {
  ...
  describe('inRange with hit tolerance', function() {
    ...
    elems.forEach(function(element) {
      ...
      const sides = element.options.sides;
      const borderWidth = element.options.borderWidth;
      const hitTolerance = element.options.hitTolerance;
      const radius = element.height / 2;
      const angle = (2 * Math.PI) / sides;
      ...
      it(`should return true inside element '${element.options.id}'`, function() {
        ...
        for (let i = 0; i < sides; i++, rad += angle) {
          const sin = Math.sin(rad);
          const cos = Math.cos(rad);
          const x = center.x + sin * (radius + halfBorder + halfTolerance - 1);
          const y = center.y - cos * (radius + halfBorder + halfTolerance - 1);
          expect(element.inRange(x, y)).withContext(`sides: ${sides}, rotation: ${rotation}, radius: ${radius}, borderWidth: ${borderWidth}, hitTolerance: ${hitTolerance}, {x: ${x.toFixed(1)}, y: ${y.toFixed(1)}}`).toEqual(true);
        }
      });
      ...
      it(`should return false outside element '${element.options.id}'`, function() {
        ...
        for (let i = 0; i < sides; i++, rad += angle) {
          ...
          const x = center.x + sin * (radius + halfBorder + halfTolerance + 1);
          const y = center.y - cos * (radius + halfBorder + halfTolerance + 1);
          expect(element.inRange(x, y)).withContext(`sides: ${sides}, rotation: ${rotation}, radius: ${radius}, borderWidth: ${borderWidth}, hitTolerance: ${hitTolerance}, {x: ${x.toFixed(1)}, y: ${y.toFixed(1)}}`).toEqual(false);
        }
      });
    });
