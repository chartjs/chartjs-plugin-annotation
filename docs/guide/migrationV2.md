 ...
 * When [stacked scales](https://www.chartjs.org/docs/latest/axes/cartesian/#common-options-to-all-cartesian-axes) are used and  `xMin`, `xMax`, `yMin` and `yMax` options are missing, the scale dimension is used instead of chart area to set the annotation dimension.
 * When [stacked scales](https://www.chartjs.org/docs/latest/axes/cartesian/#common-options-to-all-cartesian-axes) are used and  `xValue` and `yValue` options are missing, the scale dimension is used instead of chart area to set the annotation location.
